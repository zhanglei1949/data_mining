...
def train_and_evaluate(filename_list, model_name,train_ratio, timesteps = 20, feature_num = 1, epoches = 4, batch_size = 240):
    ...
    model.add(LSTM(16, dropout = 0.5, activation = 'relu', kernel_regularizer=regularizers.l2(0.01), return_sequences = True))
	...
	for filename in filename_list[:-1]:
	    ...
	    model.train_on_batch(x_train[steps*batch_size:], y_train[steps*batch_size:])
            print loss
    x_train, y_train, x_test, y_test = load_data('../data/generated_20_tick/'+filename_list[-1], 0)
    print len(y_train)
    print len(y_test)
...
def main():
    ...
    train_and_evaluate(filename_list, model_name = 'rnn_new_5.h5',train_ratio = 0.7, batch_size = 1000, epoches = 1)
